<template>
  <div id="boxx">
    <span id="typed"></span>
    <!-- <div v-html="boxx.content"
         style="margin:1.2rem auto"></div>
    <div style='float:right'><span>--来自·</span>
      <span v-html="boxx.title"></span>
    </div> -->
  </div>
</template>
  ...
  methods: {
    async getContent() {
      ...
      await axios.get('https://v1.hitokoto.cn').then((response) => {
        if (
          response.data.hitokoto !== '' &&
          response.data.hitokoto.length < 40 &&
          response.data.from_who != null
        ) {
          let author = response.data.from_who
          this.list.push({
    ...
    print() {
      const options = {
        ...
        // loop: true,
        // onTypingPaused: () => {
        //   console.log('onTypingPaused')
        // },
        // onStringTyped:() => {
        //   console.log('onStringTyped111');
        // },
...
const list = [
  ...
  {
    title: '罗素',
    content: '乞丐并不会妒忌百万富翁，但是他肯定会妒忌收入更高的乞丐',
  },
  { title: '叔本华', content: '要么庸俗，要么孤独' },
  { title: '卢梭', content: '人生而自由，却无往不在枷锁中' },
  {
    title: '严歌苓',
    content: '我发现，一个人在放弃给别人留好印象的负担之后，原来心里会如此踏实',
  },
  {
    title: '苏更生',
    content: '人表达的永远不是他所说的内容，而是渴望被理解的心情',
  },
  { title: '萧伯纳', content: '自由意味着责任，正因为如此，多数人都惧怕自由' },
  {
    title: '马尔克斯',
    content: '一个男人发现他在变老，是因为他开始像他的父亲',
  },
  {
    title: '康德',
    content:
  ...
  {
    title: '伏尔泰',
    content: '使人疲惫的不是远方的高山，而是鞋子里的一粒沙子',
  },
  {
    title: '弗洛伊德',
    content:
  ...
  {
    title: '居里夫人',
    content: "我们应该不虚度一生，应该能够说：'我已经做了我能做的事'",
  },
  { title: '唐.王之焕《登鹳雀楼》 ', content: '欲穷千里目，更上一层楼' },
  {
    title: '唐.孟浩然《江上寄山阴崔少府国辅》 ',
    content: '草木本无意，荣枯自有时',
  },
  ...
  { title: '清.赵翼《论诗》', content: '江山代有才人出，各领风骚数百年' },
  {
    title: '张鹏军',
    content:
      '知识这种东西，学过一遍和没学过是不一样的，我可能忘了它，但我的骨子里已经有了它',
  },
  ...
  {
    title: '《荀子》',
    content: '不登高山，不知天之高也；不临深溪，不知地之厚也',
  },
