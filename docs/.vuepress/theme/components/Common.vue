    ...
    <div v-else>
      <transition name="fade">
        ...
        <div v-else>
          ...
          <Password v-if="!isHasPageKey"
                    :isPage="true"></Password>
          <slot v-else></slot>
        </div>
      </transition>
    </div>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const shouldShowSidebar = computed(
      ...
      // () => root.$frontmatter.sidebar && props.sidebarItems.length > 0
    )
    const absoluteEncryption = computed(() => {
      return (
        root.$themeConfig.keyPage &&
        root.$themeConfig.keyPage.absoluteEncryption === true
      )
    })
